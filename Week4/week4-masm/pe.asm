...
expDir_header1       db 0Ah, "Ordinal       |Function RVA  |Name Ordinal  |Name RVA      |Name", 0Ah, 0h


exportDirOffset dd 1

numberFuncion dd 1
addressFuncion dd 1
addressName    dd 1
addressNameOrdinal dd 1

offsetFuncion dd 1
offsetName dd 1
offsetNameOrdinal dd 1
...
import_print endp  
 ...
 export_print proc
        ...
        mov [cur_offset1],ebx
        mov [cur_offset],ebx
        mov [exportDirOffset],ebx 
 
        invoke SetFilePointer, hFile, ebx, 0, FILE_BEGIN

        push offset expDir_header
        call StdOut 
        ...
        add dword ptr [cur_offset1], eax
        cmp esi, 10
        je _done
        inc esi
        push esi
        jmp _print_eachMember

         _done:

        mov ebx,[exportDirOffset] 
        add ebx,18h
        invoke SetFilePointer, hFile, ebx, 0, FILE_BEGIN
        ...
        invoke ReadFile, hFile, addr BytesBuffer, 4, addr BytesRead, 0
        mov ebx, dword ptr [BytesBuffer]
        mov [numberFuncion], ebx
        
        ;;addressFuncion
        invoke ReadFile, hFile, addr BytesBuffer, 4, addr BytesRead, 0
        mov ebx, dword ptr [BytesBuffer]
        mov [addressFuncion], ebx
        ...
        invoke ReadFile, hFile, addr BytesBuffer, 4, addr BytesRead, 0
        mov ebx, dword ptr [BytesBuffer]
        mov [addressNameOrdinal], ebx

         mov ebx,[addressFuncion]
         call convert_RVA_to_raw_offset
         mov [offsetFuncion], ebx

         mov ebx,[addressName]
         call convert_RVA_to_raw_offset
         mov [offsetName], ebx

         mov ebx,[addressNameOrdinal]
         call convert_RVA_to_raw_offset
         mov [offsetNameOrdinal], ebx


        push offset expDir_header1
        call StdOut
